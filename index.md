...
In order to track all of these remote connections, we created a struct to store information about every peer. This struct  includes flags for: the connection status, local and remote peer's statuses for communication, and if the local peer has a pending request. These flags are largely used to control the flow of the communication. We also store the index of the piece that both the local and remote peer wish to send. By storing these values, we can address them at a later time. The struct also stores a bitfield that corresponds to the pieces that the remote peer has. This is used during the process of piece selection. 
